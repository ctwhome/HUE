...
export const GET: RequestHandler = async ({ params, url }) => {
	try {
		...
		if (cached || requestedSessionId) {
			const projectId = cached && services().store.hasProjectMetadata(params.projectId)
				? params.projectId
				: (await authoritativeProject(params.projectId)).id;
			const stored = requestedSessionId && requestedSessionId.length <= 500
				? services().store.getSession(projectId, requestedSessionId) : null;
			const page = requestedSessionId
				? { sessions: stored ? [stored] : [], hasMore: false }
				: services().store.listSessionPage(projectId, {
				includeArchived,
				query,
				limit,
				offset
