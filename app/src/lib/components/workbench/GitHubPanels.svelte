...
<article class={`${panel} github-panel`} style:grid-area="github" style:flex={open ? undefined : '0 0 auto'} aria-label="GitHub work">
	<header
	...
	>
		{#if repositoryLink}<a
			...
			>{/if}
		<ChevronRight size={16} class={`ml-auto ${open ? 'rotate-90' : ''}`} aria-hidden="true" />
	</header>
	{#if open}
	<div class="min-h-0 flex-1 overflow-auto p-2">
		...
		<div class="grid gap-2 border-t border-border pt-2">
			<details open class="group min-w-0" aria-label="GitHub issues">
				...
				>
					<ChevronRight size={14} class="shrink-0 group-open:rotate-90" aria-hidden="true" />
