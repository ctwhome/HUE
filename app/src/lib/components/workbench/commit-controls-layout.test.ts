...
test('commit generation lives in the message input and model selection is tucked into a dialog', () => {
	...
	expect(source).toContain('aria-label="Choose commit message model"');
	expect(source).toContain('commitModelDialog?.showModal()');
	expect(source).toContain('aria-labelledby="commit-model-title"');
