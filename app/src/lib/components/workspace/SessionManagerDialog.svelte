...
>
	...
	<form onsubmit={onsave}>
		...
		<fieldset class="project-icon-field m-0 min-w-0 border-0 p-0">
			...
			<div class="project-icon-editor mt-2 grid grid-cols-[58px_minmax(0,1fr)] items-center gap-3">
				<div
					...
					<div class="project-icon-upload flex gap-1.5">
						...
						><label title="Choose a custom session image"
							><span>Choose image</span><input
								...
								onchange={onimage}
							/></label
						><button type="button" title="Use automatic session icon" onclick={() => (icon = null)}
							>Automatic</button
						>
					</div>
				</div>
		...
		<div class="edit-project-actions session-icon-actions flex flex-wrap justify-end gap-3">
			<button
				type="button"
				class="min-h-11"
				disabled
			...
			><button type="submit" class="min-h-11" title="Save Session" disabled={saving}
				>Save icon</button
			>
		</div>
	</form>
