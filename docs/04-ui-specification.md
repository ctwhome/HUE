...
Global Home, Inbox and Notifications reuse the main window without destroying the selected Space/Session state. Task, artifact, file, knowledge and context inspectors are views or splits inside the main window, not replacements for the persistent hierarchy. At narrow desktop widths the Session column becomes a drawer; on mobile, Projects, scoped Sessions, and chat become successive full-screen levels with visible Back controls and rightward swipe-back between levels. The mobile shell does not retain the desktop/global top navigation; chat instead uses a compact translucent, backdrop-blurred contextual bar so the conversation background remains perceptually continuous. A compact active-run surface may appear only while work is running, waiting or interrupted.
